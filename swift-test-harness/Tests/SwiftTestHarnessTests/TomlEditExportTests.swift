...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `TomlEdit.swiftmodule/`
...
//      `import TomlEdit` above would fail at compile time.
//
//   2. The static archive `libTomlEdit.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "TomlEdit" }` and
//      `flattenPackage = "io.github.kotlinmania.tomledit"` configuration in
...
//      via the `TomlEditLibrary` product.
//
...
final class TomlEditExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "TomlEdit swift module imported cleanly")
    }
}
