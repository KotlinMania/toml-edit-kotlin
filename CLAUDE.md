# Claude Code Project Instructions — toml-edit-kotlin

## Project Overview

This is **toml-edit-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`toml_edit`](https://crates.io/crates/toml_edit).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/toml_edit/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/toml_edit>
